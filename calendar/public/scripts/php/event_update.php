...
if($has_group) {
    $insertEvents = $mysqli->prepare(
        "UPDATE events set group_id=?, title=?, date=?, time=?, tags=? where eid=? and owner_uid=?");
    $insertEvents->bind_param('issssii',
        $group_id, $title, $date, $time, $tags, $eid, $_SESSION['user_id']); // bind params specified above
    $insertEvents->execute();
} else { // don't include grup id in insertion
    $insertEvents = $mysqli->prepare(
        "UPDATE events set title=?, date=?, time=?, tags=? where eid=? and owner_uid=?");
    $insertEvents->bind_param('ssssii',
        $title, $date, $time, $tags, $eid, $_SESSION['user_id']); // bind params specified above
    $insertEvents->execute();
}
